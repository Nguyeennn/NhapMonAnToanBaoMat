...
SERVER_HOST = '192.168.1.3' # Use '127.0.0.1' for local testing
SERVER_PORT = 65432
...
class SecureMessagingClient:
    ...
    def send_message(self, recipient_id, message_text):
        ...
        # 1. Handshake (Simulated by getting the public key from the server)
        ...
        # Create a random 3DES key
        des_key = os.urandom(24) # 24 bytes for TripleDES
        
        # Sign authentication info (ID + timestamp) with sender's private key
        ...
        # Encrypt the 3DES key with the recipient's public key (RSA OAEP)
        ...
        # Build the key exchange payload
        ...
        # 3. Encryption & Integrity Check
        
        # Create a random IV
        iv = os.urandom(8) # 8 bytes for 3DES CBC
        
        # Pad the message to be a multiple of the block size (8 bytes)
        padder = padding.PKCS7(algorithms.TripleDES.block_size).padder()
        padded_data = padder.update(message_text.encode('utf-8')) + padder.finalize()
        
        # Encrypt the message using 3DES CBC
        cipher = Cipher(algorithms.TripleDES(des_key), modes.CBC(iv), backend=self.backend)
        encryptor = cipher.encryptor()
        ciphertext = encryptor.update(padded_data) + encryptor.finalize()
        
        # Create the SHA-256 hash for integrity check
        # This hash is on (IV || ciphertext)
        hasher = hashes.Hash(hashes.SHA256(), backend=self.backend)
        hasher.update(iv + ciphertext)
        message_hash = hasher.finalize()
        
        # Sign the message hash to ensure authenticity and integrity (optional but good practice)
        # Using PSS for signing a hash
        ...
        # 4. Send the packages to the server
        full_payload = {
            "action": "send_message",
    ...
    def process_incoming_message(self, full_payload):
        ...
        # 1. Get sender's public key for verification
        ...
        # 2. Decrypt the TripleDES key using the recipient's private key (self.private_key)
        try:
        ...
        # 3. Verify the sender's signature on the authentication info
        try:
            signed_info = base64.b64decode(key_exchange_payload['signed_info'])
            # We don't have the original 'auth_info' here, but in a real app,
            # you would verify it against some known value or timestamp range.
            # This is a simplified check.
            # For this example, we'll skip the `auth_info` verification for simplicity.
        ...
        # 4. Check integrity by re-calculating the SHA-256 hash
        ...
        # Calculate local hash of IV + ciphertext
        hasher = hashes.Hash(hashes.SHA256(), backend=self.backend)
        hasher.update(iv + ciphertext)
        ...
        # 5. Authenticate the RSA signature on the message hash
        try:
        ...
        # 6. If all checks pass, decrypt the message
        try:
            ...
            # Send ACK (in this case, just a print statement)
...
if __name__ == "__main__":
    ...
    # 1. Run the server.py script in a separate terminal first.
    # 2. Run two instances of this client.py script in two different terminals.
